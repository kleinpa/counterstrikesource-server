...
the packages listed in ``base/trixie.yaml``, which pins the Debian archive to a
snapshot date. Nothing bumps that date automatically, so picking up newer
packages means editing the date in ``base/trixie.yaml`` and regenerating the
lockfile:
...
enough for the image. ``base/trixie.yaml`` explains the constraint that puts on
the snapshot date.
